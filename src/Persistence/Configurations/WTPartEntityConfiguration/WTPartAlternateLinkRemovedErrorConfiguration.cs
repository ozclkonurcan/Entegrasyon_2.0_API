...
public class WTPartAlternateLinkRemovedErrorConfiguration : IEntityTypeConfiguration<WTPartAlternateLinkRemovedErrorEntegration>
{
	public void Configure(EntityTypeBuilder<WTPartAlternateLinkRemovedErrorEntegration> builder)
	{
		builder.ToTable("Des2_WTPart_AlternateLinkRemoved_Error").HasKey(b => b.LogID);

		// Trigger olduğunu belirt
		builder.ToTable(tb => tb.HasTrigger("WTPartAlternateRemovedError_Trigger"));
		...
		builder.HasIndex(
			...
			name: "UK_WTPartAlternateLinkRemoved_Error_AnaParca_MuadilParca"
		).IsUnique();
	}
}